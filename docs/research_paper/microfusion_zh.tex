...
\textbf{按支撑强度排序的方法贡献。}整个架构由三项有实证支撑的贡献与一项假设级观察共同构成；下文统一以 C1--C4 引用，强度最高的是 C2，C3 明确是文献汇聚的假设而非方法贡献。(\textbf{C2})~\emph{对齐器阶梯天花板}（\textbf{主发现}）。\S\ref{sec:align} 报告一条由四种对齐设计构成的严格阶梯——线性 $<$ 平均 $<$ 跨架构注意力 $<$ 混合专家——在三个冻结的异构底座上同条件评测。该阶梯\emph{在混合对齐器处饱和}，而把混合嵌套进更深的层级（参数严格增加的设计）会让 MSE 反而退步 $4.7\%$。我们把这条天花板作为本文的实证贡献：它是异构底座组合问题上一条可测、可证伪的边界，\S\ref{sec:discussion} 在架构层面把它与可替代的路由方案对照讨论。(\textbf{C1})~\emph{门控式稀疏 top-K 路由作为支撑机制。}\S\ref{sec:layer}--\S\ref{sec:training} 描述的单个稀疏路由器与逐专家门控不是学到的组合矩阵，而是硬选+逐专家尺度；正是这一点让 C2 的阶梯能在 CPU 上、底座冻结的条件下训练——这一约束非平凡，因为跨架构路由历来要么要求对异构参数集做完整反向传播，要么退化为输出级集成。该机制与稀疏 MoE 文献 \citep{shazeer2017,lecun1990optimal,jacobs1991} 共享，但服务的是跨架构、而非同架构内的专家选择。(\textbf{C4})~\emph{双通路子层路由配规模异构底座}（\textbf{设计扩展}）。C1 路由器被施加到 attention 与 FFN 两个子层（\S\ref{sec:dualpath}）；两条通路可以从\emph{不同}底座——不同宽度、不同模态、不同归纳偏置——选择专家，这就是 C4 的"规模异构"半边。\S\ref{sec:fusion-exp} 的 attention--FFN 分解实验显示跨架构互补性集中在 attention 通路。规模异构选择的公开证据以双底座配置为主；规模更大的那一半使用了机构内部辅助模型，仅在作者侧保留（参见项目 README 中对 \texttt{research/scale\_heterogeneous/} 的说明）。(\textbf{C3})~\emph{数据规模依赖性是一条文献汇聚的假设}（\textbf{观察级}）。四条独立的近期路由与组合工作报告，最优的异构专家组合会随训练数据规模漂移（\S\ref{sec:discussion}）。本文的四模式阶梯在单一训练规模下完成，与该规律一致但本身并未建立规律边界；我们把 C3 作为支撑假设的观察来报告，而不是方法贡献。本文内的训练规模扫描留待未来工作。贡献结构总览见图~\ref{fig:four-inn}。

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figs/mermaid/four_innovations.pdf}
\caption{\textbf{按支撑强度排序的方法贡献}。（C2）对齐器阶梯天花板作为最强实证发现——一条严格的四模式阶梯，在嵌套时测得 $4.7\%$ 退步；（C1）门控式稀疏 top-K 路由作为支撑机制，使 C2 能在 CPU 上、底座冻结的条件下训练；（C4）双通路子层路由配规模异构底座，是 C1 在两个子层上的设计扩展；（C3）数据规模依赖性的文献汇聚假设作为观察级贡献——明确按"有前期文献支撑的假设"而非方法贡献来定位。每一项均可独立证伪，并在 §3（正文）、§4（实验）、§5（讨论）、§6（结论）中均有交叉引用锚点。}
\label{fig:four-inn}
\end{figure}
...
共享空间是异构底座相遇的地方，而它的构造方式被证明比技术栈中任何其他东西都更重要。我们评估四种复杂度递增的设计（见图~\ref{fig:ladder-chain}）：
...
\caption{\textbf{对齐器阶梯}作为一条"严格但饱和"的链条：mode A（线性投影）$\rightarrow$ B（均值对齐器）$\rightarrow$ C（跨架构 attention）$\rightarrow$ D（mixture-of-experts 对齐器）。每种 mode 在 5 种子上的 MSE 都比前一种有提升；嵌套 mixture 对齐器则退步 $4.7\%$（\S5.3），划出这条天花板。}
...
表~\ref{tab:ladder} 与图~\ref{fig:ladder-results} 展示阶梯；图是表内五种子均值的可视化，不新增量，但让次序更易读。linear $<$ mean $<$ attention $<$ mixture 的次序在每组对比中都严格成立，从线性基线到 mixture 对齐器的总爬升为 $-77.4\%$ MSE。负面结果与正面结果同样清晰：两级嵌套的混合对齐器（mixture of mixture）落在 attention 与 mixture \emph{之间}，比单层混合对齐器\emph{差} $4.7\%$。跨底座注意力值一级；第二级层级只增加参数、稀释共享概念上下文，不增加表示能力。
...
\textbf{按支撑类型排序的四个贡献，再复述一遍。}(\textbf{C2})~\emph{一条严格阶梯、在混合对齐器处饱和、嵌套后 MSE 反而退步 $4.7\%$ 的对齐器阶梯}——贡献是系统比较本身，而非任一单独模式；饱和点就是可证伪的边界。(\textbf{C1})~\emph{门控式稀疏 top-K 路由，每查询只选+门控一个专家，而非把所有专家组合成学到的混合矩阵}——让 C2 的阶梯能在 CPU 上、底座冻结条件下训练的机制；已知机制（稀疏 MoE）应用到了新目标（跨架构）。(\textbf{C4})~\emph{跨子层路由配规模异构底座}——C1 路由器被施加到 attention 与 FFN 子层，新元素是两条路由可以选不同宽度、不同模态与不同归纳偏置的底座专家；公开证据以双底座配置为主，规模更大的那一半用了内部辅助模型，仅在作者侧保留。(\textbf{C3})~\emph{文献汇聚假设：在阶梯中胜出的模式依赖于数据规模}——由四条独立的近期工作观察到（\S\ref{sec:discussion}），本文作为支撑假设的观察来报告，而非方法贡献；本文内的训练规模扫描留待未来工作。每一项都可独立证伪：C2 把阶梯折叠成单一模式（饱和点消失）；C1 把门控换成学到的组合矩阵（CPU 可训练性丢失）；C3 只在已选模式获胜的规模下训练（数据规模依赖性被反证）；C4 强迫两条路由从同一底座选专家（规模异构半边消失）。开放的前沿仍是规模。
