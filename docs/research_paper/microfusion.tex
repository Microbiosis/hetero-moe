...
\textbf{Contributions of the method, ranked by support strength.} The architecture is shaped by three operationally supported contributions and one hypothesis-tier observation; we cite them as C1--C4 throughout, with C2 carrying the strongest falsifiable finding and C3 explicitly framed as a literature-convergent hypothesis rather than a method-level contribution. (\textbf{C2})~\emph{Aligner ladder ceiling} (\textbf{main finding}). \S\ref{sec:align} reports a strict ladder of four alignment designs---linear $<$ mean $<$ cross-architecture attention $<$ mixture-of-experts---evaluated under matched conditions on three frozen heterogeneous bases. The ladder \emph{saturates} at the mixture aligner, and nesting the mixture---a design that strictly increases parameter count---regresses by $4.7\%$ MSE. We treat this ceiling as the paper's empirical contribution: it is a measured, falsifiable boundary on heterogeneous-base composition that the architecture-level discussion in \S\ref{sec:discussion} evaluates against alternative routing schemes. (\textbf{C1})~\emph{Gate-style sparse top-K routing as the enabling mechanism.} The single sparse router and per-expert gates described in \S\ref{sec:layer}--\S\ref{sec:training} are not a learned combination matrix but a hard selection plus a per-expert scale; this is what makes C2's ladder trainable on CPU with frozen bases---a non-trivial constraint given that cross-architecture routing has historically required either full backpropagation through heterogeneous parameter sets or output-level ensembling. The mechanism is shared with the sparse-MoE literature \citep{shazeer2017,lecun1990optimal,jacobs1991} but here serves cross-architecture rather than within-architecture expert selection. (\textbf{C4})~\emph{Dual-path sublayer routing with scale-heterogeneous bases} (\textbf{design extension}). The C1 router is applied to both attention and FFN sublayers (\S\ref{sec:dualpath}); the two pathways may select experts from \emph{different} bases with different widths, modalities, and inductive biases---this is the scale-heterogeneous half of C4. The attention--FFN decomposition experiment in \S\ref{sec:fusion-exp} shows that cross-architecture complementarity concentrates in the attention path. The public evidence for scale-heterogeneous selection rests on dual-base configurations; the larger-scale half of C4 uses proprietary auxiliary models and is held author-only (see the project README note on \texttt{research/scale\_heterogeneous/}). (\textbf{C3})~\emph{Data-scale dependence is a literature-convergent hypothesis} (\textbf{observation tier}). Four independent lines of recent work on routing and composition report that the optimal composition of heterogeneous experts shifts with training-data scale (\S\ref{sec:discussion}). Our four-mode ladder is run at a single training scale and is consistent with, but does not by itself establish, that regime boundary; we report C3 as an observation in support of a hypothesis rather than as a method contribution. An in-paper data-scale sweep is left to future work. Figure~\ref{fig:four-inn} summarizes the contribution structure.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figs/mermaid/four_innovations.pdf}
\caption{\textbf{Contributions of the method, ranked by support strength}. (C2) aligner ladder ceiling as the strongest empirical finding---a strict four-mode ladder with a measured $4.7\%$ regression under nesting; (C1) gate-style sparse top-K routing as the enabling mechanism that makes C2 trainable on CPU with frozen bases; (C4) dual-path sublayer routing with scale-heterogeneous bases as the design extension of C1 to two sublayers; (C3) literature-convergent data-scale dependence as the observation-tier contribution---explicitly framed as a hypothesis supported by prior work, not as a method contribution. Each is independently falsifiable and has cross-reference anchors in §3 (body), §4 (experiments), §5 (discussion), and §6 (conclusion).}
\label{fig:four-inn}
\end{figure}
...
\textbf{Four contributions, restated, ranked by support type.} (\textbf{C2})~\emph{an aligner ladder with a measured ceiling at the mixture aligner, and a $4.7\%$ regression when nested}---the contribution is the systematic comparison, not any single mode; the saturation is the falsifiable boundary. (\textbf{C1})~\emph{a gate-style sparse top-K routing that selects and gates one expert per query rather than combining all experts into a single learned mixture matrix}---the mechanism that makes C2's ladder trainable on CPU with frozen bases; a known mechanism (sparse MoE) applied to a new target (cross-architecture). (\textbf{C4})~\emph{cross-sublayer routing with scale-heterogeneous bases}, where the C1 router is applied to attention and FFN sublayers, and the new element is that the two routes may select from different bases with different widths, modalities, and inductive biases---public evidence rests on dual-base configurations; the larger-scale configuration uses proprietary auxiliary models and is held author-only. (\textbf{C3})~\emph{a literature-convergent hypothesis that the mode which wins the ladder is data-scale-dependent}, observed by four independent lines of recent work and held here as an observation in support of a hypothesis, not as a method contribution; an in-paper data-scale sweep is left to future work. Each is independently falsifiable: C2 by collapsing the ladder to a single mode (which removes the saturation finding), C1 by replacing the gate with a learned combination matrix (which loses the CPU-trainable guarantee), C3 by training only at the data scale at which the chosen mode loses (which falsifies the data-scale dependence), and C4 by forcing the two routes to pick from the same base (which removes the scale-heterogeneous half). The open frontier remains scale.
