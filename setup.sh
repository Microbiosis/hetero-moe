#!/usr/bin/env bash
# ============================================================================
# 异构 Mixture-of-Experts (Hetero-MoE) v4.0 — 可复现环境引导脚本 (幂等, 可重复执行)
# ----------------------------------------------------------------------------
# 设计目标: 即使 rootfs 被重置, 只要 /workspace/hetero-moe 源码在,
#           执行本脚本即可一键重建完整可运行环境。
# 幂等性: 每一步先检测是否已满足, 已满足则跳过, 可安全重复执行。
# 注: 本地目录 hetero-moe; Python 包名 hetero_fusion (import 路径不变)
# ============================================================================
set -euo pipefail
